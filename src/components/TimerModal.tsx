...
export const TimerModal: React.FC<TimerModalProps> = ({
  ...
  const [title, setTitle] = useState(isEditMode ? timer.title : '');
  const [description, setDescription] = useState(isEditMode ? timer.description : '');
  const [hours, setHours] = useState(isEditMode ? Math.floor(timer.duration / 3600) : 0);
  const [minutes, setMinutes] = useState(isEditMode ? Math.floor((timer.duration % 3600) / 60) : 0);
  const [seconds, setSeconds] = useState(isEditMode ? timer.duration % 60 : 0);
      ...
      <div className="bg-white rounded-lg p-6 w-full max-w-md shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <TimerButton
            ...
            submitText={isEditMode ? 'Save Changes' : 'Add Timer'}
          />
        </form>
      </div>
    </div>
